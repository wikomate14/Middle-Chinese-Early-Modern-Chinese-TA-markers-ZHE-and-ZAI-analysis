# -*- coding: utf-8 -*-
"""Analysis (2).ipynb（副本）（副本）

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JpvxW4MEXSf_q4RkY7JGJcTkhPdQmxkO
"""
...
# ---- Paths ----
BASE   = Path("/content/drive/MyDrive/Papers/S1/dataset/layered data")
OUTDIR = Path("/content/drive/MyDrive/Papers/S1/analysis_out/zai");OUTDIR.mkdir(parents=True, exist_ok=True)
...
GL_MC   = Path("/content/drive/MyDrive/Papers/S1/dataset/collection guideline - MC.txt")
GL_EMOC = Path("/content/drive/MyDrive/Papers/S1/dataset/collection guideline - EMoC.txt")
...
def _fmt_c_label(s: str) -> str:
    s = str(s)

    # 常见写法统一
    s = s.replace("世纪", "c.")
    s = re.sub(r"\bcentur(y|ies)\b", "c.", s, flags=re.IGNORECASE)   # century/centuries -> c.

    # 把类似 "13th-14th c." / "13–14 c." 这种留成 "13–14 c."
    # 先把连字符统一成 en-dash
    s = s.replace("-", "–")
...
def relabel_ci_axes(ax):

    # x tick labels：世纪 -> c.
    ticks = [t.get_text() for t in ax.get_xticklabels()]
    ax.set_xticklabels([_fmt_c_label(t) for t in ticks], rotation=45, ha="right")
...
# ====== EN label mapping & century formatter (for CA / plots) ======
import re

label_map = {
    "农学": "Agricultural Science",
    "佛教史传": "Buddhist History and Biography",
    "志怪小说": "Tales of the Supernatural",
    "佛教论典": "Buddhist treatises",
    "道教理论": "Taoist Theory",
    "志人小说": "Biographical Novels",
    "历史地理 ": "History and Geography",
    "民俗": "Folklore",
    "儒家家训": "Confucian Family Precepts",
    "佛教经典": "Buddhist Classics",
    "变文": "Transformative texts",
    "禅宗语录": "Zen aphorisms",
    "儒家语录": "Confucian aphorisms",
    "话本小说": "Story-teller’s scripts",
    "平话": "Historical narration",
    "戏曲": "Drama",
    "讲史平话": "Storytelling historical narration",
    "杂剧": "Poetic drama set to music",
    "汉语教材": "Chinese language textbook",
    "章回小说": "Chapter novel"
}
...
def _map_one(s: str) -> str:
    ...
    if s2 in label_map:
        return label_map[s2]
    # 兜底：再按“去空格”匹配一次（处理你字典里尾部空格、双空格等）
...
def _fmt_c_label(s: str) -> str:
    s = _norm(s)
    s = s.replace("世纪", "c.")
    s = re.sub(r"\bcentur(y|ies)\b", "c.", s, flags=re.IGNORECASE)
    s = s.replace("-", "–")             # 统一 en-dash
    s = re.sub(r"\s+", " ", s).strip()
    return s

def _map_ca_label(label: str) -> str:
    """
    CA 行标签常见形态：
      - "13世纪_Taoist Theory道教理论"
      - "EMoC_Chapter novel 章回小说"
    这里把 '_' 分开的每段分别做：
      1) century/epoch 格式化
      2) genre 英文化映射
    """
...
def relabel_matplotlib(ax):
    """Post-process an Axes: rewrite texts/legend using English labels."""
    # 1) 替换散点标注文字（CA 图最关键）
    for t in ax.texts:
        t.set_text(_map_ca_label(t.get_text()))

    # 2) legend（如果有）
    leg = ax.get_legend()
    if leg is not None:
        for txt in leg.get_texts():
            txt.set_text(_map_ca_label(txt.get_text()))

    return ax

# ==== ZAI | CA plotting (scheme 2: map labels at creation time) ====
# NOTE: This cell assumes you have already defined:
#   - _map_ca_label(label: str)  # your century/genre EN mapping formatter
# If not, run your "EN label mapping & century formatter" cell first.
...
# -----------------------------
# 1) CA input table constructor
# -----------------------------
def make_ca_table_zai(df: pd.DataFrame, use_century=True) -> pd.DataFrame:
    """
    Counted by (century/epoch) × genre_coarse (optional) × state, listed as 4 ZAI states.
    Also optionally merges a 'genre_full_report' summary if both genre_coarse & genre_full exist.
    """
    STATES_ZAI = ["Z-LexV", "Z-P", "Z-AdvProg?", "Z-Prog"]
    ...
    if have_coarse:
        tab = (df.groupby([group_col, "genre_coarse", "state"], dropna=False, observed=False)
                 .size()
                 .unstack("state")
                 .reindex(columns=STATES_ZAI, fill_value=0)
                 .reset_index())

        # Optional: full-genre reporting (kept for your report/debug, not needed for CA math)
        if "genre_full" in df.columns:
...
# -----------------------------------------
# 2) Plot helper: scatter + repel (scheme2)
# -----------------------------------------
def _scatter_with_labels(ax, x, y, labels, top_mask=None,
                         point_bg=14, point_fg=24, fontsize=9,
                         map_labels=True):
    """
    Draw points and annotate ONLY top_mask points.
    Scheme 2: map labels BEFORE placing them on the plot.
    """
    x = np.asarray(x)
    y = np.asarray(y)
    labels = np.asarray(labels, dtype=object)
    if top_mask is None:
        top_mask = np.ones(len(labels), dtype=bool)
    ...
    for xi, yi, lab in zip(x[top_mask], y[top_mask], labels[top_mask]):
        lab2 = _map_ca_label(lab) if map_labels else str(lab)
        t = ax.text(
            xi, yi, lab2,
...
# --------------------------------------
# 3) Small utility: micro-defuse overlap
# --------------------------------------
def _micro_defuse(xy, radius=0.015, round_dp=6):
    """
    Spread nearly-overlapping points into a small circle to reduce label collisions.
    """
    xy = np.asarray(xy, dtype=float)
    out = xy.copy()

    key = np.round(xy, round_dp)
...
# -------------------------
# 4) Run CA + draw the plots
# -------------------------
def run_ca_and_plot_zai(ca_tab: pd.DataFrame,
                        title_prefix="CA",
                        annotate_top=30,
                        show_cols=True):
    """
    CA on ZAI 4-state table. Produces:
      - Row map: (century/epoch + genre_coarse) points
      - Column map: the four states (optional)
    """
    STATES_ZAI = ["Z-LexV", "Z-P", "Z-AdvProg?", "Z-Prog"]

    # ---- Label columns (row tags) ----
    label_cols = []
    ...
    if label_cols:
        ...
        row_labels = pd.Series(range(len(ca_tab)), dtype=str)

    # ---- Numeric matrix ----
    X = ca_tab.reindex(columns=STATES_ZAI, fill_value=0).astype(float)
    ...
    rows = ca.row_coordinates(X)
    rows.index = row_labels.values

    cols = ca.column_coordinates(X)
    cols.index = X.columns  # ensure state names align to plotted points

    # Mark top-K rows by frequency
    ...
    # Coordinates
    if n_comp >= 2:
        row_xy = np.c_[rows[0].to_numpy(), rows[1].to_numpy()]
        col_xy = np.c_[cols[0].to_numpy(), cols[1].to_numpy()]
    else:
        row_xy = np.c_[rows[0].to_numpy(), np.zeros(len(rows))]
        col_xy = np.c_[cols[0].to_numpy(), np.zeros(len(cols))]

    # Micro-defuse overlaps (helps adjustText)
    ...
    if n_comp >= 2:
        ...
        ax.set_title(f"{title_prefix} rows")
        ...
        ax.set_title(f"{title_prefix} rows (1D)")
        ax.set_xlabel("Dim1"); ax.set_yticks([])
    ...
    if show_cols:
        ...
        if n_comp >= 2:
            _scatter_with_labels(
                ...
                map_labels=True  # harmless: states remain unchanged
            )
            ax.set_title(f"{title_prefix} columns (states)")
            ax.set_xlabel("Dim1"); ax.set_ylabel("Dim2")
        else:
            _scatter_with_labels(
                ax=ax,
            ...
            ax.set_title(f"{title_prefix} columns (1D)")
            ax.set_xlabel("Dim1"); ax.set_yticks([])
...
ca_mc   = make_ca_table_zai(df_mc,   use_century=True)
ca_emoc = make_ca_table_zai(df_emoc, use_century=True)

run_ca_and_plot_zai(ca_mc,   title_prefix="CA (ZAI: MC century×genre)",   annotate_top=30)
run_ca_and_plot_zai(ca_emoc, title_prefix="CA (ZAI: EMoC century×genre)", annotate_top=30)
...
def run_ca_and_plot(ca_tab: pd.DataFrame, title_prefix="CA", annotate_top=30, fname_prefix="ca"):
    ...
    if n_comp >= 2:
        ...
        plt.title(f"{title_prefix} rows"); plt.xlabel("Dim1"); plt.ylabel("Dim2"); plt.tight_layout(); plt.show()

        plt.figure()
        plt.scatter(cols[0], cols[1], s=40)
        for name, x, y in zip(cols.index, cols[0], cols[1]):
            plt.annotate(name, (x, y), fontsize=10)
        plt.title(f"{title_prefix} columns (states)"); plt.xlabel("Dim1"); plt.ylabel("Dim2"); plt.tight_layout(); plt.show()
    else:
        ...
        plt.xlabel("Dim1"); plt.yticks([]); plt.tight_layout(); plt.show()

        plt.title(f"{title_prefix} rows (1D)"); plt.xlabel("Dim1"); plt.yticks([]); plt.tight_layout(); plt.show()
        ...
        plt.title(f"{title_prefix} columns (1D)"); plt.xlabel("Dim1"); plt.yticks([]); plt.tight_layout(); plt.show()
...
import pandas as pd
print(pd.crosstab(l1["l1_bucket"], l1["state"]))

print("cond_sub" in df_all.columns, df_all["cond_sub"].notna().sum() if "cond_sub" in df_all.columns else "NO COLUMN")

# 1) 总计数表：每个 bucket 多少 token
bucket_counts = l1["l1_bucket"].value_counts().reindex(BUCKETS).fillna(0).astype(int)
print(bucket_counts)

# 2) bucket × 世纪 的交叉表（图 24–28 背后的数）
bucket_by_century = pd.crosstab(l1["l1_bucket"], l1[GROUP_COL])
bucket_by_century.to_csv(OUTDIR / "L1_bucket_by_century.csv")
print(bucket_by_century)

# 3) bucket × 最终 state 的交叉表（核查映射用，就是你上一条贴我的那张）
bucket_by_state = pd.crosstab(l1["l1_bucket"], l1["state"])
bucket_by_state.to_csv(OUTDIR / "L1_bucket_by_state.csv")
print(bucket_by_state)

cols = ["epoch", GROUP_COL, "cond_sub", "l1_bucket", "state", "context", "zai", "next"]
for b in ["L1_trigZai_V", "L1_zhengzai_V", "L1_trigZai_V_loose"]:
    sub = l1.loc[l1["l1_bucket"].eq(b), cols]
    sub.to_csv(OUTDIR / f"inspect_{b}.csv", index=False)
    print(b, len(sub))
...
# ==== ZAI-Layer1: CI (Wilson + Jeffreys) | NO persistence (no CSV, no savefig) ====
...
def jeffreys_ci(k, n, alpha=0.05):
    ...
    return (
        beta.ppf(alpha / 2, k + 0.5, n - k + 0.5),
        beta.ppf(1 - alpha / 2, k + 0.5, n - k + 0.5),
    )
...
if "GROUP_COL" not in globals():
    if "century" in l1.columns and l1["century"].notna().any():
        GROUP_COL = "century"
    elif "epoch" in l1.columns and l1["epoch"].notna().any():
        GROUP_COL = "epoch"
    else:
        raise ValueError("Cannot infer GROUP_COL: neither 'century' nor 'epoch' has usable values in l1.")

def _century_key(s):
    m = re.search(r"(\d+)", str(s))
    return int(m.group(1)) if m else 10**9

# 仅用于画图的时间顺序（不包含 NaN；NaN 组如果你想画也可以单独处理）
GROUPS = sorted(l1[GROUP_COL].dropna().astype(str).unique(), key=_century_key)

# -----------------------------
# 1) Bucketed CI table (kept in memory only)
# -----------------------------
rows = []
for gval, g in l1.groupby(GROUP_COL, dropna=False, observed=False):
    n = len(g)  # within Layer1
    vc = g["l1_bucket"].value_counts()
    for b in BUCKETS:
        k = int(vc.get(b, 0))
        wL, wC, wU = wilson_ci(k, n)
        jL, jU = jeffreys_ci(k, n)
        rows.append({
            GROUP_COL: gval, "bucket": b, "n": n, "k": k, "p": (k/n if n else np.nan),
            "W_lower": wL, "W_center": wC, "W_upper": wU,
            "J_lower": jL, "J_upper": jU
        })
...
l1_prop = (
    pd.DataFrame(rows)
      .sort_values([GROUP_COL, "bucket"])
      .reset_index(drop=True)
)

# -----------------------------
# 2) Prog-candidate line table (kept in memory only)
# -----------------------------
rows = []
for gval, g in l1.groupby(GROUP_COL, dropna=False, observed=False):
    n = len(g)
    ...
    jL, jU = jeffreys_ci(k, n)
    rows.append({
        GROUP_COL: gval, "series": "Prog-candidate", "n": n, "k": k, "p": (k/n if n else np.nan),
        "W_lower": wL, "W_center": wC, "W_upper": wU,
        "J_lower": jL, "J_upper": jU
    })
...
# -----------------------------
# 3) Plot: one per bucket (no saving)
# -----------------------------
for b in BUCKETS:
    d = (l1_prop[l1_prop["bucket"] == b]
         .set_index(GROUP_COL)
    ...
    ax.fill_between(x, yL, yU, alpha=.2)
    ax.set_title(f"Layer1 {b} over {GROUP_COL}")
    ax.set_xticks(x)
    ax.set_xticklabels(GROUPS, rotation=45, ha="right")
    ax.set_ylabel("Proportion (within Layer1)")
...
# -----------------------------
# 4) Plot: merged Prog-candidate line (no saving)
# -----------------------------
d = l1_progline.set_index(GROUP_COL).reindex(GROUPS)
x = np.arange(len(GROUPS))
y  = d["p"].to_numpy()
...
ax.plot(x, y, marker="o")
ax.fill_between(x, yL, yU, alpha=.2)
ax.set_title(f"Layer1 Prog-candidate over {GROUP_COL}")
ax.set_xticks(x)
ax.set_xticklabels(GROUPS, rotation=45, ha="right")
ax.set_ylabel("Proportion (within Layer1)")
...
print("[ok] ZAI-Layer1 CI plotted.")
...
# ---- build table (NO CSV persistence) ----
ca_L1 = make_ca_table_L1(l1, use_century=True)


def _scatter_with_labels(ax, x, y, labels, top_mask=None,
                         point_bg=18, point_fg=34, fontsize=10, alpha_bg=0.25, alpha_fg=0.85):
    x = np.asarray(x); y = np.asarray(y); labels = np.asarray(labels)
    if top_mask is None: top_mask = np.ones_like(x, dtype=bool)

    ax.scatter(x[~top_mask], y[~top_mask], s=point_bg, alpha=alpha_bg)
    ax.scatter(x[top_mask],  y[top_mask],  s=point_fg, alpha=alpha_fg)

    texts = []
    for xi, yi, lab in zip(x[top_mask], y[top_mask], labels[top_mask]):
        lab2 = _map_ca_label(lab)  # <-- EN mapping + century formatter
        t = ax.text(
            xi, yi, lab2, fontsize=fontsize, ha="center", va="center",
    ...
    if texts:
        adjust_text(
            ...
            expand_points=(1.6,1.6), expand_text=(1.6,1.6),
            autoalign='xy',
            arrowprops=dict(arrowstyle="-", lw=0.5, alpha=0.35, shrinkA=6, shrinkB=6),
        )


def run_ca_and_plot_generic(ca_tab: pd.DataFrame,
                            title_prefix="CA",
                            annotate_top=30,
                            dpi=260,
                            fig_scale=1.0):
    ...
    if X.shape[0]==0 or X.shape[1]==0:
        print("[warn] CA input all zeros"); return
    ...
    # Only mark the first K rows by frequency
    freq = X.sum(axis=1).to_numpy()
    keep = np.argsort(-freq)[:min(int(annotate_top), len(freq))]
    top_mask = np.zeros(len(freq), bool); top_mask[keep] = True
    ...
    if n_comp >= 2:
        # Rows
        fig, ax = plt.subplots(figsize=(8.5*fig_scale, 6.5*fig_scale), dpi=dpi)
        _scatter_with_labels(
            ax,
            x=rows[0].to_numpy(), y=rows[1].to_numpy(),
            labels=rows.index.to_numpy(), top_mask=top_mask,
        ...
        ax.set_xlabel("Dim1"); ax.set_ylabel("Dim2")
        plt.tight_layout()
        relabel_matplotlib(ax)  # <-- 兜底：再映射一次（含 legend）
        plt.show()
        ...
        ax.set_title(f"{title_prefix} — columns")
        ax.set_xlabel("Dim1"); ax.set_ylabel("Dim2")
        plt.tight_layout()
        relabel_matplotlib(ax)
        plt.show()
        ...
        fig, ax = plt.subplots(figsize=(9.0*fig_scale, 3.2*fig_scale), dpi=dpi)
        _scatter_with_labels(
            ax,
            x=rows[0].to_numpy(), y=jitter,
            labels=rows.index.to_numpy(), top_mask=top_mask
...
# —— CA graphs of l1 ——
run_ca_and_plot_generic(
    ca_L1,
    title_prefix="CA (Layer1: period × genre × buckets)",
    annotate_top=30,
    dpi=280,
    fig_scale=1.1
)
...
X = ca_L1[num_cols].apply(pd.to_numeric, errors="coerce").fillna(0).to_numpy(dtype=float)

# ---- figure geometry: make it readable ----
n_rows, n_cols = X.shape
fig_h = max(6.0, 0.60 * n_rows)         # 你可以再加大，比如 0.70
fig_w = max(10.0, 1.25 * n_cols)        # 列少就不会太宽
fig, ax = plt.subplots(figsize=(fig_w, fig_h), dpi=240)  # dpi 提高清晰度

# ---- contrast: avoid one huge cell flattening everything ----
pos = X[X > 0]
vmax = np.percentile(pos, 98) if pos.size else 1.0       # 用 98 分位压缩极端值
X_plot = np.clip(X, 0, vmax)

im = ax.imshow(
    X_plot,
    aspect="auto",
    interpolation="nearest",      # 防止像素插值变糊
    cmap="cividis",               # 比 viridis 更“耐打印”，你也可换 "magma"
    norm=PowerNorm(gamma=0.55, vmin=0, vmax=vmax)  # 提升暗部对比
)

# ---- y labels: period × genre -> EN (using your mapper) ----
ax.set_yticks(np.arange(n_rows))
row_lab_cols = [c for c in ["century","epoch","genre_coarse"] if c in ca_L1.columns]
if row_lab_cols:
    row_labels = ca_L1[row_lab_cols].astype(str).agg("_".join, axis=1).tolist()
else:
    row_labels = [str(i) for i in range(n_rows)]

row_labels_en = [_map_ca_label(s) for s in row_labels]
ax.set_yticklabels(row_labels_en, fontsize=10)

# ---- x labels ----
ax.set_xticks(np.arange(n_cols))
ax.set_xticklabels([_map_ca_label(c) for c in num_cols], rotation=35, ha="right", fontsize=10)

# ---- cosmetics ----
cbar = fig.colorbar(im, ax=ax, fraction=0.02, pad=0.02)
cbar.set_label("Count", rotation=90)

ax.set_title("L1 wide matrix preview (rows = period × genre; cols = buckets)")
ax.set_xlabel("Buckets")
ax.set_ylabel("Period × Genre (EN)")
